"""
kimipay.py — KimiPay payment gateway integration.
API endpoint: https://kimipay.in/api
...
def _base():
    url = (fb.cfg("kimipay_base_url") or "").strip().rstrip("/")
...
def create_order(amount: int, order_sn: str, description: str = "",
    ...
    sig_params = {k: v for k, v in params.items() if k not in ("callback_url", "description", "customer_email")}
    params["signature"] = _sign(sig_params, api_key)
    ...
    try:
        ...
        if d.get("status") == 1:
            return {
                "success":          True,
                "kimipay_order_id": d["data"]["order_id"],
