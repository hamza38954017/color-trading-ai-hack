"""
support_bot.py — Customer Support Bot for Predictor.
Messages stored at Firebase /support/{chat_id}/messages/{msg_id}.
Admin notifications sent to SUPPORT_NOTIFY_CHAT_IDS.
"""
import telebot
from telebot import types
import datetime, os, time, io
import firebase_helper as fb
from config import SUPPORT_NOTIFY_CHAT_IDS, SUPPORT_BOT_TOKEN, SUPPORT_GREETING
...
def send_support_notify(chat_id: str, user_name: str, text: str, msg_type: str = "text"):
    ...
    try:
        ...
        for admin_id in ids:
            try:
                support_bot.send_message(admin_id, notify_text, parse_mode="Markdown", reply_markup=mk)
...
def get_user_photo_url(user_id):
    try:
        ...
        if photos and photos.photos:
            ...
            tok  = _get_token()
            return f"https://api.telegram.org/file/bot{tok}/{info.file_path}"
    except Exception as e:
        print(f"[PHOTO] {e}")
    return ""
...
BLOCKED_MSG = "🚫 You have been blocked from support. Contact admin."
...
def handle_text(msg):
    ...
    store_message(cid, mid, {
        ...
        "ts": now_ts(), "read": False, "delivered": True, "edited": False,
    })
...
def handle_photo(msg):
    ...
    store_message(cid, mid, {
        ...
        "text": caption, "caption": caption, "file_id": photo.file_id, "file_url": file_url,
        "type": "photo", "from": "user", "time": now_str(), "ts": now_ts(),
...
def handle_video(msg):
    ...
    store_message(cid, mid, {
        "msg_id": mid, "chat_id": cid, "user_name": uname, "username": un,
        "text": caption, "caption": caption, "file_id": msg.video.file_id, "file_url": file_url,
        "type": "video", "from": "user", "time": now_str(), "ts": now_ts(),
        "read": False, "delivered": True, "edited": False,
    })
    mark_admin_msgs_read(cid)
...
def handle_document(msg):
    ...
    store_message(cid, mid, {
        "msg_id": mid, "chat_id": cid, "user_name": uname, "username": un,
        "text": caption, "caption": caption, "file_id": file_id, "file_url": file_url,
        "file_name": file_name, "type": "document", "from": "user",
        "time": now_str(), "ts": now_ts(), "read": False, "delivered": True, "edited": False,
    })
    mark_admin_msgs_read(cid)
...
def admin_reply(chat_id: str, text: str, image_url: str = "") -> dict:
    if not support_bot: return {"error": "Support bot not running"}
    try:
        ...
        fb.put(f"support/{chat_id}/messages/admin_{mid}", {
            "msg_id": f"admin_{mid}", "chat_id": chat_id, "text": text,
            "image_url": image_url, "type": "photo" if image_url else "text",
            "from": "admin", "time": now_str(), "ts": now_ts(),
            "read": False, "delivered": True, "edited": False,
        })
        fb.patch(f"support/{chat_id}/meta", {
            "last_message": text or "[image]", "last_time": now_str(),
...
def admin_send_file(chat_id: str, file_bytes: bytes, filename: str,
    ...
    try:
        ...
        mid      = str(m.message_id)
        fb.put(f"support/{chat_id}/messages/admin_{mid}", {
            "msg_id": f"admin_{mid}", "chat_id": chat_id, "text": caption, "file_url": file_url,
            "file_name": filename, "type": ftype, "from": "admin",
            "time": now_str(), "ts": now_ts(), "read": False, "delivered": True, "edited": False,
        })
        fb.patch(f"support/{chat_id}/meta", {
            "last_message": caption or f"[{ftype}]", "last_time": now_str(),
            "last_ts": now_ts(), "unread": 0,
        })
